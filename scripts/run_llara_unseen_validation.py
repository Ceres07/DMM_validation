...
DEFAULT_REPORT_DIR = DMM_VALIDATION_ROOT / "reports/llara_unseen_model6_vs_model8"
...
def build_feature_table(args: argparse.Namespace, obs: pd.DataFrame, bbox: tuple[float, float, float, float]) -> pd.DataFrame:
    from PaddockTS.query import Query

    path = args.outdir / "llara_model_input_features.csv"
    ...
    q = Query(
        ...
        stub="llara_unseen_" + "_".join(f"{v:.3f}" for v in bbox).replace(".", "p").replace("-", "m"),
    )
    ...
    smips = sample_smips_features(
        ...
        bbox,
        quiet=args.quiet,
        cache_path=args.outdir / "llara_smips_point_timeseries.csv",
        force=args.force,
...
def predict_model6(args: argparse.Namespace, obs: pd.DataFrame, features: pd.DataFrame) -> pd.DataFrame:
    from emt.model6 import model as model6_module
    from emt.persist import load_model

    path = args.outdir / "model6_model_agnostic_predictions.csv"
    if path.exists() and not args.force:
        return pd.read_csv(path)
...
def predict_model8(
    ...
    path = args.outdir / "model8_process_model_agnostic_predictions.csv"
    if path.exists() and not args.force:
        return pd.read_csv(path)
    ...
    rain, pet, lons, lats = _forcing_grid(bbox, sim_start, end, args.model8_step_deg, verbose=not args.quiet)
    ...
    table["model_name"] = "model8_process"
    table["prediction_status"] = np.where(valid, "ok", "missing_feature")
    table.to_csv(path, index=False)
    return table
