#!/usr/bin/env python3
"""Sparse local-calibration experiment across Esdale, Tarrawarra and Llara.

Purpose
-------
This replaces the Phenode/time-series local-calibration experiment with a
site-to-site dense-campaign experiment. The practical question is deliberately
property-management flavoured:

    If a landowner can afford only one or a few local soil-moisture sensors,
    where should they go, and does that tiny local information budget improve
...
DEFAULT_OUTDIR = DMM_VALIDATION_ROOT / "outputs" / "three_site_sparse_local_calibration"
DEFAULT_REPORT = DMM_VALIDATION_ROOT / "reports" / "three_site_sparse_local_calibration_report.md"
DEFAULT_REPORT_FIGDIR = DMM_VALIDATION_ROOT / "reports" / "three_site_sparse_local_calibration" / "figures"
...
REPORT_FIGURES = (
    ...
    (
        ...
        "Strict spatio-temporal random sparse-sensor learning curves",
        "Median RMSE gain from random sparse local sensors when calibration and validation are separated in space and time.",
    ),
    (
        "one_sensor_strategy_comparison_rmse_gain.png",
        "One-sensor selection strategy comparison",
...
SITE_PATHS = {
    ...
    / "model6_model8_combined_predictions_valid.csv",
...
METHODS = ["global", "bias_offset", "affine", "residual_ridge"]
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--budgets", default="1,2,3,5", help="Comma-separated calibration point budgets.")
    parser.add_argument("--random-reps", type=int, default=30)
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=True,
        help="Write one-sensor strict-block predictions for plotting/inspection.",
    )
...
def summarize_metrics(metrics: pd.DataFrame) -> pd.DataFrame:
    group_cols = ["site", "block", "selection_strategy", "calibration_points", "base_model", "model_track", "method"]
...
def process_vs_statistical(metrics: pd.DataFrame) -> pd.DataFrame:
    ...
    keys = ["site", "block", "selection_strategy", "calibration_points", "replicate", "method"]
    ...
    group_cols = ["site", "block", "selection_strategy", "calibration_points", "method"]
    return (
...
def make_figures(outdir: Path, baseline: pd.DataFrame, summary: pd.DataFrame, responsiveness: pd.DataFrame) -> None:
    ...
    for ax, ylab in zip(axes, ["RMSE (%)", "NSE/R²"]):
        ax.set_xticks(x)
    ...
    if not core.empty:
        fig, axes = plt.subplots(len(sites), 1, figsize=(9.5, max(3.2 * len(sites), 5)), sharex=True)
    ...
    if not resp.empty:
        fig, axes = plt.subplots(len(sites), 1, figsize=(9.5, max(3.0 * len(sites), 5)), sharex=True)
        axes = np.atleast_1d(axes)
        for ax, site in zip(axes, sites):
...
def write_report(
    ...
    best_one = (
        core.sort_values("rmse_median")
        .groupby(["site", "base_model"], as_index=False)
        .first()[
            [
                "site",
                "base_model",
    ...
    tarrawarra_note = (
        "Tarrawarra model6 should be read with a special caveat: the historical "
        "SMIPS inputs in the existing Tarrawarra run are zero, so model6 there is "
        "closer to a missing-coarse-anchor ablation than a normal model6 run."
    )
    figures_md = render_report_figures(out_path, report_figdir)

    body = f"""# Three-site sparse local calibration: Esdale, Tarrawarra and Llara
...
{markdown_table(best_one, max_rows=30)}
    ...
    for site, path in SITE_PATHS.items():
        ...
        for budget in budgets:
            for strategy in SELECTION_STRATEGIES:
                reps = args.random_reps if strategy == "random" else 1
