...
SRC = ROOT / "src"
LOCAL_CALIBRATION_DIR = ROOT / "scripts" / "analyses" / "local_calibration"
...
SITE_NOTES = {
    ...
    "Tarrawarra": (
        "Very dense 1995/96 campaign. Raw sub-cell campaign points are aggregated "
        "to the model prediction grid cell by date before validation, so the "
        "validation unit matches the raster support. The existing model6 run has "
...
def ordered_sites(values: pd.Series | list[str] | set[str]) -> list[str]:
    """Keep manuscript site order, then append any separate/new sites."""
    seen = {str(v) for v in values}
    ordered = [site for site in PREFERRED_SITE_ORDER if site in seen]
...
STAGE1_FIGURES = [
    ...
    (
        "figures/stage1/dem_point_overlays/dem_points_overlay_gallery.png",
...
def plot_stage1_site_skill(overall: pd.DataFrame, fig_dir: Path) -> Path:
    ...
    sites = ordered_sites(overall["site"])
...
def run_gallery(script_name: str, out_path: Path) -> tuple[Path | None, str | None]:
    script_dirs = {
        "plot_dense_model6_model8_gallery.py": ROOT / "scripts" / "sites" / "esdale",
        "plot_tarrawarra_model6_model8_gallery.py": ROOT / "scripts" / "sites" / "tarrawarra",
    }
    script = script_dirs.get(script_name, ROOT / "scripts") / script_name
...
def write_unified_report(
    ...
    notable_terrain_report = report_columns(
        notable_terrain_report,
        [
            "site",
            "base_model",
            "terrain_var",
...
{markdown_table(paired_overall_report, max_rows=20)}
