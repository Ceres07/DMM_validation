...
\author{Dmitry Grishin\thanks{[affiliation; email]} \and
...
Root-zone soil-moisture products are frequently evaluated against sparse
...
critical. Here we use five independent validation datasets in two stages: firstly, to test
whether the final statistical and process-based models developed in the
companion paper preserve paddock-scale soil-moisture patterns. Secondly, 
local calibration sensitivity is tested across the five sites to determine 
the minimum ground-truth sensor deployment to achieve good (NSE > 0.3) local model 
performance. Both models are evaluated on a model-agnostic terrain and climate covariate 
table to allow for direct comparison of model performance. 

The datasets span modern dense campaign points,
...
Overall, the models transfer moderately to the independent validation sites, with
the best result obtained for the Tarrawarra model 8 prediction which achieved an NSE of 0.07 
and a Pearson r of 0.8. Untrained models consistently displayed wetness state compression
error, where dry states where too wet and wet states too dry. Error was not significantly 
stratified by terrain covariates or season. 

Local calibration performed best when using a sensor selection strategy that mirrored 
field knowledge of wet and dry parts of the landscape. Using a spatial and temporal block, 
local calibration increased model performance from NSE around 0 to NSE ranging from 0.15 to 0.4. 
Excluding the historical sites with no SMIPS anchor, the process model 8 responded more positively
to local calibration than the statistical model 6. 
...
\item \textbf{Seasonal, wetness-state and terrain-conditioned bias.}  Are poor
  ...
  model-agnostic terrain/soil strata?
...
\caption{Available validation-site terrain context.  Esdale, Nerrigundah, Llara
WE, Llara WW and MRI use the 30\,m Copernicus/model terrain grid; Tarrawarra
uses the converted 5\,m campaign DEM.  Red points are soil-moisture validation
supports: probe locations for Esdale, Llara and MRI, and aggregated
model-grid-cell centroids for Tarrawarra and Nerrigundah.}
...
associated model-agnostic covariate strata.  This allows \modelSix{} and \modelEight{} to be
compared without relying on either model's internal representation.

For Tarrawarra and Nerrigundah, the current \modelSix{} prediction
tables have a \smips{}-zero/coarse-anchor caveat because historical SMIPS
coverage was unavailable. Thus interpretation of \modelSix{} skill at these sites 
is limited to model covariate stratified error and temporal dynamics rather than
absolute error. 
...
Static model-agnostic covariates & elevation, slope, TWI, HLI, accumulation, aspect
components, SLGA soil properties \\
Dynamic model-agnostic covariates & SMIPS summaries where available, antecedent rainfall,
...
Before interpreting validation metrics, it is useful to see examples of
the gridded objects being scored.
Figures~\ref{fig:esdale-dry-wet}--\ref{fig:llara-gridded} show observed dry and wet
conditions where gridded products are already staged for the report.  MRI is
retained as a sparse continuous-probe validation in this draft, so it is shown
through the DEM context map and time-series diagnostics rather than a gridded
wet/dry map panel.  These panels are intended as a visual orientation to the map
products; all scores below are calculated from matched validation supports
rather than from visual inspection. 

Notably, however, visual inspection suggests that model 8 has a more pronounced 
terrain distribution of water, more closely resembling the TWI than model 6 outputs. 
...
estimate, \modelSix{} and \modelEight{}.  Mean predicted soil moisture across
validation supports was 14.8\,\% and 15.5\,\% on the dry date, and 20.3\,\% and
24.8\,\% on the wet date, for \modelSix{} and \modelEight{} respectively.}
...
\modelEight{}.  Mean predicted soil moisture across aggregated validation
supports was 21.7\,\% and 23.4\,\% on the dry date, and 23.6\,\% and 31.6\,\%
on the wet date, for \modelSix{} and \modelEight{} respectively.  \caution{The
historical \smips{} input is affected by the zero-valued coarse-anchor caveat.}}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.8\textwidth]{nerrigundah_dry_wet_model6_model8_gallery.png}
\caption{Nerrigundah gridded prediction overview for the observed driest
(17~September~1997; mean observed soil moisture 22.1\,\%) and wettest
(22~September~1997; 37.0\,\%) campaign dates after aggregation to model-grid
support.  Mean predicted soil moisture across aggregated validation supports
was 23.9\,\% and 26.6\,\% on the dry date, and 24.9\,\% and 28.5\,\% on the wet
date, for \modelSix{} and \modelEight{} respectively.}
\label{fig:nerrigundah-dry-wet}
\end{figure}
...
\caption{Llara gridded prediction examples for the WE and WW paddocks, shown
separately.  In WE, the dry example is 6~November~2021
(mean observed 20.9\,\%; \modelSix{} 26.7\,\%; \modelEight{} 25.5\,\%) and the
wet example is 21~October~2022 (67.7\,\%; 34.3\,\%; 37.2\,\%).  In WW, the dry
example is 20~February~2022 (18.4\,\%; 26.4\,\%; 21.5\,\%) and the wet example
is 21~October~2022 (57.8\,\%; 38.9\,\%; 35.8\,\%).}
...
scale, matching closely to the unseen site cross validation results from the companion paper.
Across the five retained datasets, pooled \nse{} is near zero for most
site/model combinations, however the Pearson correlations are consistently moderate, 
ranging from 0.237 to 0.853, with the exception of Nerrigundah which has very low variability
across the time period. Therefore models often track some spatial or temporal ordering without
reproducing the level offset; a modelling challenge noted as being possibly due to 
sensor-level measurement error and thus challenging to resolve (Adeel Ansari, 2026).

Esdale and Nerrigundah have the lowest RMSE, approximately 6--6.5 volumetric percentage points, but
...
mean-level bias.  Alongside support level scoring, they
make temporal drift and wet/dry compression visually apparent. Visually, the model outputs match the observed dynamics well. The predicted time series
is generally compressed compared to the observed time series, but the predicted 
direction of change is generally correct. 
...
spatial means over available validation points for each date.  The MRI panel is
scaled to 15--60\,\% soil moisture to keep the observed and predicted traces
readable across the long probe record.}
...
drought stress and paddock management decisions.
...
observation; positive values indicate overprediction.  Esdale and Nerrigundah
have limited seasonal coverage, while Llara and MRI provide the clearest
multi-year seasonal bias diagnostics.}
...
quartiles test whether models compress extremes.  The main diagnostic is
whether predictions move toward the centre of the observed distribution, i.e.
overpredicting dry supports and underpredicting wet supports.}
\label{fig:wetness-bias}
\end{figure}

\FloatBarrier

\subsection{Terrain and model-agnostic covariate stratification of error}
\label{sec:stage1-terrain}

The validation supports allow error to be stratified by model-agnostic
covariates that drive the gridded predictions, including elevation, slope, TWI,
HLI, accumulation, aspect components, SLGA soil properties and antecedent
...
The global-scaled static terrain--soil PCA asks whether the validation sites
occupy different absolute terrain--soil settings
(Fig.~\ref{fig:pca-static-global-site}).  This view preserves site means, so
between-site separation should be interpreted as environmental transfer
distance rather than within-site model failure.  PC1 explains 32.4\,\% of the
pooled static terrain--soil variation and mainly contrasts sandier,
higher-elevation, higher-bulk-density supports against clayier, high-HLI and
east-facing supports.  PC2 explains 20.2\,\% and is dominated by soil available
water capacity, eastness, HLI, bulk density, slope and TWI.  PC3 explains a
further 14.6\,\% and loads most strongly on TWI and accumulation, which is a
reminder that the two-axis PCA figure is a useful orientation, not a complete
terrain-space representation.

\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{static_terrain_soil_global_site_pca.png}
\caption{Global-scaled static terrain--soil PCA of validation supports.  PCA
inputs are elevation, slope, aspect components, TWI, HLI, accumulation and SLGA
soil properties.  Colours identify sites and arrows show the dominant loadings,
so the figure describes how the retained validation sites occupy different
terrain--soil covariate space before any residual overlay is added.}
\label{fig:pca-static-global-site}
\end{figure}
...
After removing site-level covariate means, residuals show only weak-to-moderate
alignment with common terrain/covariate gradients.  The strongest direct
association in the current diagnostic tables is HLI versus support-level bias at
approximately $r=0.375$, which corresponds to roughly 14\,\% of variance in a
simple univariate sense.  The static terrain--soil PCA does not show a clean
``bad supports here, good supports there'' separation, and PC1+PC2 explain only
about 41\,\% of the site-standardised static terrain--soil variation.  Several
stronger associations appear on PC3 or in model/site-specific combinations, and
the covariate ranking is diffuse across exposure/aspect, slope, soil texture,
TWI and accumulation.  This suggests that model error is partly structured, but
that the structure is site-dependent rather than a universal terrain-error
mechanism shared across all validation sites.  Given the introduced historical
coarse-anchor caveat for \modelSix{} at Tarrawarra and Nerrigundah, the two
models also show no large, simple difference in terrain-space clustering of
error.
...
Support-level overlap metrics provide a direct companion to the PCA diagnostic
(Table~\ref{tab:model-failure-overlap}).  For each site, RMSE and bias were
calculated separately for \modelSix{} and \modelEight{} at each validation
support after pooling over available dates.  The correlations in
Table~\ref{tab:model-failure-overlap} therefore ask whether the supports that
are difficult for one model are also difficult for the other.  Positive
correlations at every site indicate that the two models tend to perform poorly
at broadly similar supports; the clearest differences between models are in
error magnitude and bias rather than in wholly distinct spatial failure
locations.

\begin{table}[htbp]
\centering
\small
\caption{Support-level model-failure overlap by site.  RMSE and bias
correlations compare \modelSix{} and \modelEight{} across validation supports
within each site.  Higher positive values indicate that the two models tend to
have high error or similar signed bias at the same supports.}
\label{tab:model-failure-overlap}
\begin{tabular}{lrrr}
\toprule
Site & Supports & RMSE correlation & Bias correlation \\
\midrule
Esdale & 77 & 0.476 & 0.858 \\
Llara & 32 & 0.796 & 0.950 \\
MRI & 18 & 0.986 & 0.990 \\
Nerrigundah & 128 & 0.525 & 0.986 \\
Tarrawarra & 169 & 0.897 & 0.873 \\
\bottomrule
\end{tabular}
\end{table}
...
\item \textbf{Residual ridge layer:} residual model using selected model-agnostic
  covariates.
\end{enumerate}
...
The final protocol retains four point-selection strategies, but assigns them
different evidentiary roles.  \texttt{random} selects eligible calibration
supports without using landscape or model information and is retained as a
conservative baseline comparator.  \texttt{landscape\_wetdry\_prior} selects
alternating wet and dry extremes from a model-agnostic landscape wetness score
derived from terrain, soil, coarse soil-moisture anchor and exposure/elevation
covariates.  It represents the kind of deployable prior a landholder or analyst
could construct before collecting local soil-moisture measurements.
\texttt{global\_prediction\_extremes} selects alternating extremes of the
uncalibrated model prediction mean.  This is also deployable at an unseen site,
but it lets the base model identify where it expects the wettest and driest
supports to occur.  Finally, \texttt{field\_knowledge\_wetdry\_proxy} selects
alternating extremes of the observed chronic wet/dry ranking.  Because this
ranking is calculated from the validation observations themselves, it is not a
blind deployment strategy.  It is retained only as an optimistic upper-bound
proxy for unusually accurate field knowledge or a preliminary dense survey, and
is excluded from the main learning-curve figures and final minimum-information
budget table.  The main Stage~2 inference therefore focuses on the two
deployable non-random strategies, \texttt{landscape\_wetdry\_prior} and
\texttt{global\_prediction\_extremes}; the random and observed wet/dry proxy
strategies are kept as context and sensitivity checks.
...
against model-agnostic diagnostics such as TWI, HLI, elevation, soil texture,
...
The models developed in the companion paper are validated at regional scale with sites 
represented by a single sensor placement. This data structure is valuable for model building
as it allows a broad range of environmental gradients to be included in the training process
without over representing a single densely sampled site. The limitation of this training
protocol is that within site spatial variability is learnt from regional scale variability.
The goal of this paper is to evaluate how relationships learnt from regional scale transfer to 
within site spatial and seasonal accuracy. 

We used summaries of seasonal and wetness state bias, 
as well as decomposed support-level terrain-stratified error using PCA to identify whether the models
sufficiently capture temporal dynamics as well as spatial organisation of local soil moisture. 
Broadly, both models suffer from a compression problem where the wettest and driest deciles are compressed
towards the mean. Seasonal analysis also demonstrated that residuals are not stable through time. 
In terms of within-site error associated with particular topographic and soil positions, e.g. ridges and gullies, 
sandy and clayey, there was no clear clustering of high residuals in the feature space between either models. 
...
When RMSE and bias are evaluated within sites and compared between models at paired support 
points both models fail in the same ways. No site had a negative correlation between support-level
RMSE or bias, suggesting that the models cannot offer complementarity at the within-site level. 
...
The 30m soil moisture model developed in the companion paper and independently validated 
here is intended to by used by land owners and managers who are 
interested in fine scale soil moisture mapping. While the 'global' model performs moderately
at unseen sites, managers can choose to implement on ground soil moisture sensors to improve
model performance at their site. This local calibration can be seen as a site-specific adapatation
layer rather than a retraining of the base model. The adaptation layers are residual offsets applied 
to the model prediction. 

The analyses in the first stage of this paper help to inform 
where sensor deployment can yield the greatest benefit for local model calibration. Overall, 
while the models do not fail in any particular part of the terrain space, they do struggle to predict
wetness extremes. Thus, local calibration benefits most from deployment strategies that target
wetness extremes. This can be estimated in two ways to guide deployment: using the global model
to predict over the property then characterise which pixels are the wettest and driest, or using 
terrain and soil covariates to define which areas are most likely to be wet and dry given their 
topographic position and soil water holding capacity. 
...
- talk about how this evaluation is still limited in scope to south eastern australia. 
...
\item Full terrain/model-agnostic covariate strata tables.
...
\section{Dynamic model-agnostic covariate PCA}
\label{app:dynamic-pca}

The main text uses static terrain--soil PCA to keep the site-space argument
focused.  The corresponding global-scaled PCA for dynamic model-agnostic
covariates is retained here because it folds in terrain, soil, coarse SMIPS
context and antecedent meteorological summaries.  It therefore helps separate
static terrain--soil separation from broader terrain+climate transfer distance,
without overloading the main terrain-residual figure.

\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth,height=0.82\textheight,keepaspectratio]{model_agnostic_covariates_global_site_pca.png}
\caption{Appendix global-scaled PCA of dynamic model-agnostic covariates.  The
feature set combines static terrain--soil variables with coarse SMIPS summaries,
antecedent rainfall, PET and VPD summaries.  Colours identify validation sites
and arrows show the dominant loadings.}
\label{fig:app-dynamic-global-pca}
\end{figure}
