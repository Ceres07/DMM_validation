...
than interpolated point surfaces.  Esdale and Tarrawarra dry/wet raster
examples are shown at the start of Section~\ref{sec:stage1}
(Figs.~\ref{fig:esdale-dry-wet} and \ref{fig:tarrawarra-dry-wet}) so that
readers see the map products before the scalar validation summaries.  The
Llara paddocks are shown separately because the two fields have distinct
terrain footprints and sensor layouts.
...
strictly held-out supports as more local information is supplied.  RMSE gain is
useful because it has the same units as soil moisture
(Fig.~\ref{fig:learning-curves-rmse}), but it can hide whether the calibrated
model becomes efficient relative to the observed variance.  The corresponding
\nse{} learning curve (Fig.~\ref{fig:learning-curves-nse}) therefore shows
whether calibration moves the model toward modest but interpretable efficiency
thresholds such as \nse{} $\approx 0.2$--0.3.
...
blocking.  Positive values indicate lower RMSE after calibration.}
...
\caption{Sparse local-calibration learning curves expressed as held-out
\nse{} under strict spatial+temporal blocking.  Dashed and dotted reference
lines mark \nse{} = 0.2 and 0.3, respectively.  These thresholds are not
universal performance targets, but they are useful for asking how much local
information is needed before calibration becomes meaningfully better than the
...
In the current strict random-sensor experiment, \modelEight{} generally responds
...
\modelSix{} from the same sparse local information budget under random
strict-block calibration.}
